...
- [结果输出与实验有效性](文档/结果输出与实验有效性.md)：模型—参数—结果精简报告、分区 JSON、专项诊断和实验有效性。
- [统计输出精简验收](文档/统计输出精简验收.md)：指标清理、四标准前后对照和工具链回归。
- [配置与参数联动](文档/配置与参数联动.md)：schema 3、复制即用、主输入/派生/审计分类、版本迁移及数值变化。
- [配置重构与交付手册验收](文档/配置重构与交付手册验收.md)：本次改造、逐节实际复跑、发现的修复项与证据位置。
...
日常运行默认打印精简的模型、参数、性能三部分。`summary` 和 `full` 均使用该报告；
