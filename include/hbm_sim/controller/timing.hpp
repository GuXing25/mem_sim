...
// 它维护 table-driven constraints、tFAW 和 WCK 窗口，是把 timing 作用域显式化的核心。
...
class TimingEngine {
 public:
  ...
  // reset() 会丢弃所有历史 timing 状态。Controller 构造和测试重新配置 spec 后调用。
  void reset(const DramSpec& spec);

  // row_state/column_state 使用 spec.row_bus_scope/spec.column_bus_scope 指定的作用域，
  // 因此 HBM 可以按 pseudo-channel 建 bucket，LPDDR 可以按 channel 或 subchannel 建 bucket。
  TimingScopeState& row_state(const DramSpec& spec, const DecodedAddress& decoded);
  const TimingScopeState& row_state(const DramSpec& spec, const DecodedAddress& decoded) const;
  TimingScopeState& column_state(const DramSpec& spec, const DecodedAddress& decoded);
  const TimingScopeState& column_state(const DramSpec& spec, const DecodedAddress& decoded) const;
  // bank_group_state 固定按 BankGroup 作用域，用于 same/different BG 的 nCCD/nRRD 差异。
  TimingScopeState& bank_group_state(const DramSpec& spec, const DecodedAddress& decoded);
  const TimingScopeState& bank_group_state(const DramSpec& spec, const DecodedAddress& decoded) const;
  // wck_state 使用 spec.wck_scope。LPDDR6 可按 subchannel 建 WCK 窗口，LPDDR5 可按 channel。
  TimingScopeState& wck_state(const DramSpec& spec, const DecodedAddress& decoded);
  const TimingScopeState& wck_state(const DramSpec& spec, const DecodedAddress& decoded) const;
  ...
  // ACT/ACT1/REFpb/REFdb 这类会占用 activation window 的事件调用 record_activate()。
  void record_activate(const DramSpec& spec, const DecodedAddress& decoded, Cycle clk);
  // faw_ready() 只检查窗口中是否少于 4 个事件；具体 nRRD 仍由其他 gate/constraint 控制。
  bool faw_ready(const DramSpec& spec, const DecodedAddress& decoded) const;
  // LPDDR 数据命令是否落在 WCK ready window 内。
  bool wck_ready_for_data(const DramSpec& spec, const DecodedAddress& decoded, Cycle clk) const;

 private:
  // scope_count/scope_index 是本模块最核心的地址到 bucket 映射逻辑。
  // 如果未来补更多 JEDEC 层级，例如 bank-pair 或 die-stack scope，应从这里扩展。
  std::size_t scope_count(const DramSpec& spec, TimingScope scope) const;
  std::size_t scope_index(const DramSpec& spec, TimingScope scope, const DecodedAddress& decoded) const;
  Cycle timing_delay(const DramSpec& spec, int cycles) const;
  // mutable_scope/scope_state 给 table-driven constraint 使用，因为每条 constraint
  // 自己声明作用域，不能只走 row/column/wck 固定路径。
  TimingScopeState& mutable_scope(const DramSpec& spec, TimingScope scope, const DecodedAddress& decoded);
  const TimingScopeState& scope_state(const DramSpec& spec, TimingScope scope, const DecodedAddress& decoded) const;

  // 下面这些 vector 都是“按某个 DRAM 层级展开的 timing bucket 数组”。
  // bucket 内保存 ready time 和历史事件；DecodedAddress 通过 scope_index() 映射到数组下标。
  ...
  // row_scopes_/column_scopes_ 的作用域来自 spec.row_bus_scope/spec.column_bus_scope。
