...
int parity_bits_for_data_bits(std::size_t data_bits) {
  int parity_bits = 1;
  while ((std::uint64_t{1} << parity_bits) < data_bits + static_cast<std::size_t>(parity_bits) + 1) {
    parity_bits++;
  }
  return parity_bits;
}
...
double command_energy_pj(const StorageModelOptions& options,
  ...
  switch (command) {
    ...
    case Command::ACT1: return options.act1_energy_pj;
    case Command::ACT2: return options.act2_energy_pj;
    case Command::PRE:
    case Command::PREPB: return options.pre_energy_pj;
    case Command::PREAB: return options.preab_energy_pj;
    case Command::CASRD:
    case Command::CASWR: return options.cas_energy_pj;
    case Command::RD:
    case Command::RDA: return options.read_energy_pj +
                               options.read_energy_per_byte_pj * static_cast<double>(payload_bytes);
    case Command::WR:
    case Command::WRA: return options.write_energy_pj +
                               options.write_energy_per_byte_pj * static_cast<double>(payload_bytes);
    case Command::REFPB: return options.refpb_energy_pj;
    case Command::REFDB: return options.refdb_energy_pj;
    case Command::REFAB: return options.refab_energy_pj;
    case Command::RFMPB: return options.rfmpb_energy_pj;
    case Command::RFMAB: return options.rfmab_energy_pj;
    case Command::MRW:
    case Command::MRR:
...
StorageModelOptions calibrate_power_from_idd(const DramSpec& spec, StorageModelOptions options) {
  ...
  const double devices = options.idd_devices_per_rank > 0.0 ? options.idd_devices_per_rank : 1.0;
  const double burst_cycles = options.idd_burst_cycles > 0.0
  ...
  options.read_energy_pj =
      options.idd_vdd * positive_current_delta(options.idd4r_ma, options.idd3n_ma) * burst_ns * devices;
  options.write_energy_pj =
      options.idd_vdd * positive_current_delta(options.idd4w_ma, options.idd3n_ma) * burst_ns * devices;
  options.read_energy_per_byte_pj = 0.0;
  options.write_energy_per_byte_pj = 0.0;
  options.refab_energy_pj =
      options.idd_vdd * positive_current_delta(options.idd5ab_ma, options.idd3n_ma) * trfc_ns * devices;
  options.refpb_energy_pj =
      options.idd_vdd * positive_current_delta(options.idd5pb_ma, options.idd3n_ma) * trfcpb_ns * devices;
...
void set_decoded_key(DecodedAddress& decoded, const std::string& key, const std::string& value) {
  ...
  else if (key == "rank") decoded.rank = parsed;
  else if (key == "bg" || key == "bank_group") decoded.bank_group = parsed;
  else if (key == "bank") decoded.bank = parsed;
  else if (key == "row") decoded.row = parsed;
  else if (key == "col" || key == "column") decoded.column = parsed;
}

bool is_decoded_key(const std::string& key) {
  return key == "ch" || key == "channel" ||
         key == "pc" || key == "pseudo_channel" || key == "pseudo" ||
         key == "sid" || key == "rank" ||
         key == "bg" || key == "bank_group" ||
         key == "bank" || key == "row" ||
         key == "col" || key == "column";
}
...
std::size_t StorageKeyHash::operator()(const StorageKey& key) const {
  return static_cast<std::size_t>(key_of({key.stack,
                                          key.channel,
                                          key.pseudo_channel,
                                          key.sid,
                                          key.rank,
...
DataCheckResult DataValidator::check_read(Cycle cycle,
  ...
  while (mismatch_offset < expected.size() &&
         mismatch_offset < actual.size() &&
...
void DataValidator::dump_text(const std::string& path) const {
  ...
  out << "# cycle request_id address initialized forwarded stack ch pc sid rank bg bank row col layer tile_x tile_y "
  ...
  for (const auto& mismatch : mismatches_) {
    ...
    if (mismatch.block) {
      out << mismatch.block->version << ' '
          ...
          << (mismatch.block->ecc_valid ? "true" : "false") << ' '
          << "0x" << std::hex << mismatch.block->ecc_hamming << std::dec << ' '
...
MemoryImage::MemoryImage(DramSpec spec,
...
PhysicalAddress MemoryImage::physical_address(Address address, const DecodedAddress* decoded) const {
  ...
  if (options_.floorplan_enabled) {
    const int pseudo_channels = spec_.has_value() ? std::max(1, spec_->org.pseudo_channels) : 1;
    const int ranks = spec_.has_value() ? std::max(1, spec_->org.ranks) : 1;
    const int bank_groups = spec_.has_value() ? std::max(1, spec_->org.bank_groups) : 1;
    ...
    physical.floorplan_rows = std::max(1, floorplan_rows_per_slice * pseudo_channels * sid_count * ranks);
  ...
  const int row_count = spec_.has_value() ? std::max(1, spec_->org.rows) : std::max(1, d.row + 1);
  const int column_count = spec_.has_value() ? std::max(1, spec_->org.columns) : std::max(1, d.column + 1);
  ...
  physical.microbump_x = std::clamp(physical.tile_x % std::max(1, options_.microbumps_x),
                                    0,
                                    std::max(1, options_.microbumps_x) - 1);
  physical.microbump_y = std::clamp((physical.tile_y + physical.layer) % std::max(1, options_.microbumps_y),
                                    0,
                                    std::max(1, options_.microbumps_y) - 1);
  return physical;
}

StorageKey MemoryImage::storage_key(Address address, const DecodedAddress* decoded) const {
...
PhysicalStorageStats MemoryImage::storage_stats() const {
  ...
  stats.bytes_allocated = stats.lines_allocated * line_size_;
  ...
  stats.thermal_peak_temp_c = options_.thermal_ambient_c;
  ...
  if (scan_topology) {
    for (Address base : backend_->all_addresses()) {
      ...
      ranks.insert(key_of({p.stack, p.channel, p.pseudo_channel, p.sid, p.rank}));
      bank_groups.insert(
          key_of({p.stack, p.channel, p.pseudo_channel, p.sid, p.rank, p.bank_group}));
      banks.insert(key_of(
          {p.stack, p.channel, p.pseudo_channel, p.sid, p.rank, p.bank_group, p.bank}));
      rows.insert(key_of({p.stack, p.channel, p.pseudo_channel, p.sid, p.rank,
                          p.bank_group, p.bank, p.row}));
      columns.insert(key_of({p.stack, p.channel, p.pseudo_channel, p.sid, p.rank,
                             p.bank_group, p.bank, p.row, p.column}));
      subarrays.insert(key_of({p.stack, p.channel, p.pseudo_channel, p.sid, p.rank,
                               p.bank_group, p.bank, p.subarray}));
      ...
      cells.insert(key_of({p.stack, p.channel, p.pseudo_channel, p.sid, p.rank,
                           p.bank_group, p.bank, p.subarray, p.mat_id, p.cell_x, p.cell_y}));
      microbumps.insert(key_of({p.stack, p.layer, p.microbump_x, p.microbump_y}));
  ...
  for (const auto& [key, tile] : thermal_tiles_) {
    ...
    if (!saw_tile || tile.temperature_c > stats.thermal_peak_temp_c) {
      ...
      stats.thermal_hotspot_layer = tile.physical.layer;
      stats.thermal_hotspot_x = tile.physical.thermal_x;
      stats.thermal_hotspot_y = tile.physical.thermal_y;
    }
  }
  if (!thermal_tiles_.empty()) {
    stats.thermal_avg_temp_c = temp_sum / static_cast<double>(thermal_tiles_.size());
  }
...
std::optional<DataBlockMetadata> MemoryImage::metadata(Address address, const DecodedAddress* decoded) const {
  Address base = line_base(address);
...
bool MemoryImage::load_backend_line(Address base,
                                    DataBlock& block,
  ...
  block.storage_key = decoded == nullptr ? line.storage_key : storage_key(base, decoded);
...
FloorplanKey MemoryImage::floorplan_key(const PhysicalAddress& physical) const {
  return FloorplanKey{physical.stack, physical.layer, physical.tile_x, physical.tile_y};
}

ThermalGridKey MemoryImage::thermal_grid_key(const PhysicalAddress& physical) const {
  return ThermalGridKey{physical.stack, physical.layer, physical.thermal_x, physical.thermal_y};
}
...
void MemoryImage::maybe_inject_ecc_error(DataBlock& block) {
  ...
  if (block.version == 0 || (block.version % static_cast<std::uint64_t>(options_.ecc_inject_period)) != 0) {
    return;
  }
...
void MemoryImage::check_ecc_shadow(DataBlock& block) {
  if (!options_.ecc_shadow_enabled || !options_.ecc_check_on_read || !block.ecc_valid) {
    return;
  }
  ecc_checked_reads_++;
  ...
  std::optional<std::size_t> data_bit = data_bit_for_code_position(syndrome, block.bytes.size() * 8);
...
void MemoryImage::couple_thermal_neighbor(const ThermalGridKey& source_key,
  ...
  ThermalGridKey neighbor_key{source_key.stack, source_key.layer + dz, source_key.x + dx, source_key.y + dy};
  ...
  auto& dst = thermal_tiles_[neighbor_key];
  if (dst.events == 0) {
...
void MemoryImage::apply_thermal_event(const PhysicalAddress& physical, Cycle cycle, double energy_pj) {
...
DataBlock* MemoryImage::row_buffer_block(Address base,
  ...
  auto rb_it = row_buffers_.find(bank_key);
  if (rb_it == row_buffers_.end() || !rb_it->second.open || !(rb_it->second.row_key == row_key)) {
    row_buffer_misses_++;
    return nullptr;
  }
...
const DataBlock* MemoryImage::row_buffer_block(Address base, const DecodedAddress* decoded) const {
  if (decoded == nullptr) {
    return nullptr;
  }
  StorageKey line_key = storage_key(base, decoded);
...
ByteVector MemoryImage::read(Address address,
                             std::size_t size,
                             bool* initialized,
                             const DecodedAddress* decoded) {
...
ByteVector MemoryImage::read_initialized_mask(Address address,
                                              std::size_t size,
                                              const DecodedAddress* decoded) const {
  ...
  while (copied < size) {
    ...
    if (const DataBlock* rb_block = row_buffer_block(base, decoded)) {
      std::copy_n(rb_block->initialized_mask.begin() + static_cast<std::ptrdiff_t>(offset),
                  static_cast<std::ptrdiff_t>(chunk),
                  out.begin() + static_cast<std::ptrdiff_t>(copied));
    } else {
      DataBlock block;
      if (load_backend_line(base, block, decoded)) {
        std::copy_n(block.initialized_mask.begin() + static_cast<std::ptrdiff_t>(offset),
                    static_cast<std::ptrdiff_t>(chunk),
                    out.begin() + static_cast<std::ptrdiff_t>(copied));
      }
    }
...
void MemoryImage::write(Address address,
  ...
  while (copied < data.size()) {
    ...
    if (wrote_any) {
      ...
      if (row_buffer_target) {
        row_buffer_writes_++;
        row_buffers_[bank_key_from_storage_key(target->storage_key)].dirty = true;
      }
    }
    if (row_buffer_target) {
...
void MemoryImage::load_text(const std::string& path) {
  ...
  while (std::getline(in, line)) {
    ...
    for (std::size_t i = 0; i < tokens.size(); i++) {
      ...
      if (key == "addr" || key == "address") {
      ...
      } else if (key == "init" || key == "initialized" || key == "initialized_mask") {
        ...
        has_decoded = true;
                 ...
                 key == "tile_id" || key == "floorplan_cols" || key == "floorplan_rows" ||
                 key == "thermal_x" || key == "thermal_y" || key == "thermal_z" ||
                 ...
                 key == "ecc_valid" || key == "ecc_hamming" || key == "ecc_overall" ||
                 key == "ecc_uncorrectable" || key == "ecc_parity_bits") {
    ...
    if (!address.has_value()) {
      throw std::runtime_error("memory image line " + std::to_string(lineno) + " missing address");
    }
    if (data.empty()) {
      throw std::runtime_error("memory image line " + std::to_string(lineno) + " missing data");
    }
    ...
    while (copied < data.size()) {
      ...
      std::size_t offset = line_offset(current);
      std::size_t chunk = std::min(data.size() - copied, line_size_ - offset);
      const DecodedAddress* line_decoded = has_decoded && base == first_base ? &decoded : nullptr;
...
void MemoryImage::dump_text(const std::string& path) const {
  ...
  for (Address base : bases) {
    ...
    out << format_address(base)
        ...
        << " cell_x=" << p.cell_x
        << " cell_y=" << p.cell_y
        << " microbump_x=" << p.microbump_x
        << " microbump_y=" << p.microbump_y
        ...
        << " thermal_x=" << p.thermal_x
        << " thermal_y=" << p.thermal_y
...
void MemoryImage::dump_csv(const std::string& path) const {
  ...
  for (Address base : bases) {
    ...
    const PhysicalAddress& p = block.physical;
    out << format_address(base) << ','
        << bytes_to_hex(block.bytes) << ','
        << bytes_to_hex(block.initialized_mask) << ','
        << (block.initialized ? "true" : "false") << ','
        << block.version << ','
        << block.last_writer_request_id << ','
        << block.last_write_cycle << ','
        << format_u32_hex(block.checksum) << ','
        << (block.ecc_valid ? "true" : "false") << ','
        << "0x" << std::hex << block.ecc_hamming << std::dec << ','
        << (block.ecc_overall ? "true" : "false") << ','
        << (block.ecc_uncorrectable ? "true" : "false") << ','
        << p.channel << ','
        << p.pseudo_channel << ','
        << p.sid << ','
        << p.rank << ','
...
void MemoryImage::dump_binary(const std::string& path) const {
  ...
  if (!out) {
    throw std::runtime_error("failed to open binary memory image output: " + path);
  }
...
void MemoryImage::load_binary(const std::string& path) {
  ...
  if (!in) {
    throw std::runtime_error("failed to open binary memory image input: " + path);
  }
  ...
  if (magic != kBinaryMagic) {
    throw std::runtime_error("binary memory image has invalid magic number: " + path);
  }
  if (version != kBinaryVersion) {
    throw std::runtime_error(
        "binary memory image version mismatch: expected " +
        std::to_string(kBinaryVersion) + ", got " + std::to_string(version));
  }
  ...
  if (file_line_size != static_cast<std::uint32_t>(line_size_)) {
    throw std::runtime_error(
        "binary memory image line_size mismatch: file=" +
        std::to_string(file_line_size) + " current=" + std::to_string(line_size_));
  }
  ...
  for (std::uint32_t i = 0; i < entry_count; i++) {
    ...
    if (!in) {
      throw std::runtime_error(
          "binary memory image: failed to read entry " + std::to_string(i) +
          " at address 0x" +
          [base]() {
            std::ostringstream os;
            os << std::hex << base;
...
void MemoryImage::dump_thermal_text(const std::string& path) const {
  ...
  out << "# model floorplan_enabled=" << (options_.floorplan_enabled ? "true" : "false")
      ...
      << " thermal_coupling_enabled=" << (options_.thermal_coupling_enabled ? "true" : "false")
      ...
      << '\n';
  out << "# stack layer thermal_x thermal_y thermal_z tile_x tile_y grid_x grid_y tile_id temperature_c energy_pj events "
      << "ch pc sid rank bg bank row col subarray mat_x mat_y mat_id cell_x cell_y microbump_x microbump_y\n";
