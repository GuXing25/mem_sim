...
struct ValidatorScopeState {
  // next_command 与 TimingEngine::TimingScopeState 同义：在某个 scope bucket 内，
  // 每种命令下一次允许发射的最早 cycle。Validator 独立维护它，避免信任在线状态。
...
struct ValidatorBankState {
  ...
  // 这样 trace validation 能成为真正独立的后处理插件，而不是在线状态的回放打印。
  int open_row = -1;
...
std::size_t scope_count(const DramSpec& spec, TimingScope scope) {
  ...
  // 这里刻意复制 TimingEngine 的 scope_count/scope_index，而不是直接复用私有方法。
  // Validator 作为“独立插件”应能发现 TimingEngine 自身遗漏的错误。
  switch (scope) {
...
std::size_t scope_index(const DramSpec& spec, TimingScope scope, const DecodedAddress& d) {
  const Organization& o = spec.org;
...
void add_error(CommandValidationReport& report, const IssuedCommand& issued, const std::string& message) {
...
bool is_faw_event(Command command) {
  // 当前模型把 ACT/ACT1 和 per-bank/dual-bank refresh 都作为 activation-window 事件。
  // 这是保守近似，和 TimingEngine::record_activate() 保持一致。
...
int hbm_bank_key(const DramSpec& spec, const DecodedAddress& decoded) {
  return (decoded.sid * std::max(1, spec.org.bank_groups) + decoded.bank_group) *
...
std::pair<std::size_t, std::size_t> channel_bank_range(const DramSpec& spec, const DecodedAddress& decoded) {
  // all-bank 命令在本项目中按 channel 作用域执行，所以 validator 只扫描该 channel
  // 对应的 flat bank 范围，而不是整套 MemorySystem 的所有 channel。
  std::size_t banks_per_channel = static_cast<std::size_t>(std::max(1, spec.banks_per_channel()));
  int channel = std::clamp(decoded.channel, 0, std::max(1, spec.org.channels) - 1);
  std::size_t begin = static_cast<std::size_t>(channel) * banks_per_channel;
  std::size_t end = std::min(scope_count(spec, TimingScope::Bank), begin + banks_per_channel);
  return {begin, end};
}
...
bool any_bank_busy_in_channel(const DramSpec& spec,
  ...
  auto [begin, end] = channel_bank_range(spec, decoded);
...
void close_channel_banks(const DramSpec& spec,
...
DecodedAddress dual_bank_partner(const DramSpec& spec, const DecodedAddress& decoded) {
  DecodedAddress partner = decoded;
  // 与 CommandExecutor 保持同一 dual-bank pairing 近似。后续如果导入完整 JEDEC
  // bank-pair 表，需要同时更新 executor、controller idle 检查和 validator。
...
void record_hbm_rising_row_command(const DramSpec& spec,
  ...
  pairing.bank_key = command_meta(issued.command).all_bank ? -1 : hbm_bank_key(spec, issued.decoded);
  pairing.next_pairing_falling_edge = issued.cycle + (issued.command == Command::ACT ? 3 : 1);
}

void validate_and_replay_bank_state(const DramSpec& spec,
                                    std::vector<ValidatorBankState>& banks,
  ...
  switch (issued.command) {
    case Command::ACT:
      require_state(!spec.split_activate, "ACT is illegal for split-activate standards");
      ...
      break;
    case Command::ACT1:
      require_state(spec.split_activate, "ACT1 is illegal for non split-activate standards");
      ...
      break;
    case Command::ACT2:
      require_state(spec.split_activate, "ACT2 is illegal for non split-activate standards");
    ...
    case Command::PREAB:
      require_state(any_bank_busy_in_channel(spec, banks, issued.decoded),
                    "PREab requires at least one busy bank in its channel");
      close_channel_banks(spec, banks, issued.decoded);
      break;
    case Command::CASRD:
    case Command::CASWR:
      require_state(spec.lpddr_family, "CAS_RD/CAS_WR are only legal for LPDDR-family standards");
    ...
    case Command::REFPB:
    case Command::RFMPB:
      require_state(!bank.busy(), "per-bank REF/RFM requires a closed target bank");
      break;
    case Command::REFDB: {
      require_state(spec.lpddr_dual_bank_refresh, "REFdb requires LPDDR dual-bank refresh support");
      DecodedAddress partner = dual_bank_partner(spec, issued.decoded);
      const ValidatorBankState& partner_bank = banks[scope_index(spec, TimingScope::Bank, partner)];
      require_state(!bank.busy(), "REFdb requires the target bank to be closed");
      require_state(!partner_bank.busy(), "REFdb requires the paired bank to be closed");
      break;
    }
    case Command::REFAB:
    case Command::RFMAB:
      // all-bank refresh/RFM 不能在 channel 内仍有打开行或 split-activate 未完成时发生。
      // Controller 在线路径会先插入 PREab；Validator 用这条规则检查 trace 是否真的做到。
      require_state(!any_bank_busy_in_channel(spec, banks, issued.decoded),
                    "all-bank REF/RFM requires all banks in the channel to be idle");
      break;
    case Command::MRW:
    case Command::MRR:
      require_state(!any_bank_busy_in_channel(spec, banks, issued.decoded),
    ...
    case Command::WCKSYNC:
      ...
      require_state(!channel_state.self_refresh, "WCK_SYNC cannot be issued in self-refresh");
      require_state(!channel_state.wck_retrain_required,
                    "WCK_SYNC cannot replace required WCK_TRAIN after DVFS");
      break;
    case Command::WCKTRAIN:
      require_state(spec.lpddr_family, "WCK_TRAIN requires LPDDR-family standard");
      require_state(!any_bank_busy_in_channel(spec, banks, issued.decoded),
      ...
      break;
    case Command::DVFS:
      require_state(spec.lpddr_family, "DVFS transition requires LPDDR-family standard");
      require_state(!any_bank_busy_in_channel(spec, banks, issued.decoded),
    ...
    case Command::SREFEN:
      ...
      break;
    case Command::SREFEX:
      require_state(channel_state.self_refresh, "SREFEX requires self-refresh state");
...
static CommandValidationReport validate_single_stack_trace(
    const DramSpec& spec, const std::vector<IssuedCommand>& trace) {
  CommandValidationReport report;
  // bank_states 重放命令语义；constraint_scopes 重放表驱动 timing；activation_scopes
  // 重放 tFAW；wck_scopes 重放 LPDDR WCK 窗口。四者共同覆盖协议合法性。
  std::vector<ValidatorBankState> bank_states(scope_count(spec, TimingScope::Bank));
  std::array<std::vector<ValidatorScopeState>, 6> constraint_scopes;
  for (TimingScope scope : {TimingScope::Channel, TimingScope::PseudoChannel, TimingScope::Sid,
                            TimingScope::Rank, TimingScope::BankGroup, TimingScope::Bank}) {
    constraint_scopes[scope_slot(scope)].assign(scope_count(spec, scope), {});
  }
  std::vector<ValidatorScopeState> activation_scopes(scope_count(spec, spec.activation_scope));
  std::vector<ValidatorScopeState> wck_scopes(scope_count(spec, spec.wck_scope));
  std::vector<ValidatorScopeState> channel_states(scope_count(spec, TimingScope::Channel));
  std::vector<ValidatorHbmPairingState> hbm_pairing_states(scope_count(spec, TimingScope::Channel));

  auto scope_state_for = [&](TimingScope scope, const DecodedAddress& decoded) -> ValidatorScopeState& {
    return constraint_scopes[scope_slot(scope)][scope_index(spec, scope, decoded)];
  };
  ...
  for (const auto& issued : trace) {
    ...
    if (spec.dual_command_bus) {
      ...
      if (issued.bus == BusClass::Unified) {
        add_error(report, issued, "HBM dual-command mode issued command on unified bus");
      }
    }

    if (spec.hbm_edge_pairing) {
      report.edge_checks++;
      // HBM edge pairing 检查使用 trace cycle 奇偶：当前模型约定奇数 tick 为 rising，
      // 偶数 tick 为 falling。falling edge 只能用于受限 PRE；严格模式还会
      // 重放上一条 rising row command 留下的 pairing window。
      if (meta.column_command && (issued.cycle % 2) == 0) {
        add_error(report, issued, "column command issued on falling edge");
      }
      if ((issued.cycle % 2) == 0 &&
          issued.command != Command::PREPB && issued.command != Command::PREAB) {
        add_error(report, issued, "falling-edge command is not PREpb/PREab");
      }
      auto& pairing_state = hbm_pairing_states[scope_index(spec, TimingScope::Channel, issued.decoded)];
      if ((issued.cycle % 2) == 0) {
    ...
    report.state_checks++;
    validate_and_replay_bank_state(spec, bank_states, channel_states, report, issued);

    for (const auto& constraint : spec.timing_constraints) {
      if (constraint.window > 0 || !contains_command(constraint.following, issued.command)) {
        continue;
      }
      report.timing_constraint_checks++;
      ValidatorScopeState& state = scope_state_for(constraint.scope, issued.decoded);
    ...
    if (is_faw_event(issued.command)) {
      ...
      ValidatorScopeState& state = activation_scopes[scope_index(spec, spec.activation_scope, issued.decoded)];
      Cycle window = timing_delay(spec, std::max(1, spec.timing.nFAW));
      while (!state.recent_acts.empty() && issued.cycle >= state.recent_acts.front() &&
             issued.cycle - state.recent_acts.front() >= window) {
        state.recent_acts.pop_front();
      }
      if (state.recent_acts.size() >= 4) {
        // 当前命令加入前窗口内已有 4 个事件，说明这条命令会成为第五个，违反 tFAW。
    ...
    if (spec.lpddr_family && spec.lpddr_wck_mode != LpddrWckMode::AlwaysOn) {
      ValidatorScopeState& wck = wck_scopes[scope_index(spec, spec.wck_scope, issued.decoded)];
      if (is_data_command(issued.command) &&
          !(issued.cycle >= wck.wck_ready_at && issued.cycle < wck.wck_active_until)) {
        report.wck_window_checks++;
        // LPDDR 的 RD/WR 不只要求 bank row-hit，还必须落在 WCK ready window。
        // 这个检查专门捕捉 CAS_RD/CAS_WR 过早、过晚或缺失的问题。
        add_error(report, issued, "LPDDR data command outside WCK ready window");
      } else if (is_data_command(issued.command)) {
        report.wck_window_checks++;
      }
      if (issued.command == Command::CASRD || issued.command == Command::CASWR ||
          issued.command == Command::WCKSYNC) {
        // CAS 命令建立初始 WCK window；后续 RD/WR 会把 active_until 延长到
        // 覆盖数据 burst 和 postamble。
        const int sync_latency = issued.command == Command::WCKSYNC ? spec.timing.nWCKSYNC : spec.timing.nWCK2CK;
        wck.wck_ready_at = issued.cycle + timing_delay(spec, std::max(1, sync_latency));
        wck.wck_active_until =
            issued.cycle + timing_delay(spec, std::max(spec.timing.nWCKPST, sync_latency + 1));
      } else if (issued.command == Command::WCKTRAIN) {
        wck.wck_ready_at = issued.cycle + timing_delay(spec, std::max(1, spec.timing.nWCKTRAIN));
        wck.wck_active_until =
            wck.wck_ready_at + timing_delay(spec, std::max(1, spec.timing.nWCKPST));
      } else if (issued.command == Command::RD || issued.command == Command::RDA) {
        wck.wck_active_until = std::max(
            wck.wck_active_until,
            issued.cycle + timing_delay(spec, spec.timing.nCL + spec.timing.nBL + spec.timing.nWCKPST));
      } else if (issued.command == Command::WR || issued.command == Command::WRA) {
        wck.wck_active_until = std::max(
            wck.wck_active_until,
            issued.cycle + timing_delay(spec, spec.timing.nCWL + spec.timing.nBL + spec.timing.nWCKPST));
      }
    }

    for (const auto& constraint : spec.timing_constraints) {
      if (constraint.window > 0 || !contains_command(constraint.preceding, issued.command)) {
        continue;
      }
      ValidatorScopeState& state = scope_state_for(constraint.scope, issued.decoded);
      Cycle ready = issued.cycle + timing_delay(spec, std::max(0, constraint.latency));
...
CommandValidationReport validate_command_trace(const DramSpec& spec,
  ...
  for (const auto& command : trace) {
    if (std::find(stack_ids.begin(), stack_ids.end(), command.stack_id) == stack_ids.end()) {
      stack_ids.push_back(command.stack_id);
    }
  }
  if (stack_ids.empty()) stack_ids.push_back(0);

  CommandValidationReport total;
  for (int stack_id : stack_ids) {
    std::vector<IssuedCommand> local;
    for (const auto& command : trace) {
      if (command.stack_id == stack_id) local.push_back(command);
    }
    ...
    for (const auto& error : part.errors) {
      total.errors.push_back("stack " + std::to_string(stack_id) + ": " + error);
    }
  }
  return total;
}
